...
- 驱动：`tools/balance_bot.gd` 无头自动游玩（启发式走位/避弹/避爆炸域/避 hazard/环绕走位/索敌开火/概率翻滚/商店买红心/事件一律接受/三选一贪心），hero=mage（m4-c2 --hero 参数，缺省 vanguard），headless 墙钟速率（引擎 time_scale 对本作逐拍定步长逻辑无加速效用，物理拍=真实玩家体验时长）
...
| 单局时长(均值/中位) | 1.2 / 1.4 min | 25.0~35.0 min（单局口径） | 第 1 层子集，仅参考 |

## 逐局

| seed | 结果 | 止步 | 房间 | 击杀 | 时长(min) | 金币 | 蓝晶 | 买红心 | 增益选择 | 首杀可获 | 死因 |
|---|---|---|---|---|---|---|---|---|---|---|---|
| 2201 | death | 第1层 | 3 | 25 | 1.4 | 0 | 0 | 0 | - | 否 | 弩兵的弹幕（combat） |
| 2202 | death | 第1层 | 4 | 23 | 1.0 | 2 | 0 | 0 | - | 否 | 双刀蜥人的接触冲撞（elite） |
...
|---|---|---|---|
| A1 杂兵 TTK | 4.7 / 10.1 s (n=43) | ≤2.0 s | 偏离 |
| 精英击杀 | 4.5 / 11.0 s (n=5) | （参考） | - |
| 战斗房时长 | 16.8 / 27.1 s (n=6) | 20.0~40.0 s | 偏离（目标 20.0~40.0 s） |
