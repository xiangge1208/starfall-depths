...
| 单局时长(均值/中位) | 1.8 / 3.0 min | 25.0~35.0 min（单局口径） | 第 1 层子集，仅参考 |

## 逐局

| seed | 结果 | 止步 | 房间 | 击杀 | 时长(min) | 金币 | 蓝晶 | 买红心 | 增益选择 | 首杀可获 | 死因 |
|---|---|---|---|---|---|---|---|---|---|---|---|
| 2308 | death | 第1层 | 10 | 56 | 3.0 | 37 | 15 | 0 | - | 否 | 冰蛛的接触冲撞（miniboss） |
| 2309 | death | 第1层 | 0 | 2 | 0.7 | 0 | 0 | 0 | - | 否 | 弩兵的弹幕（combat） |
...
|---|---|---|---|
| A1 杂兵 TTK | 2.9 / 14.4 s (n=42) | ≤2.0 s | 偏离 |
| 精英击杀 | 3.4 / 25.1 s (n=8) | （参考） | - |
| 小 Boss 击杀 | 5.3 / 7.2 s (n=8) | （参考） | - |
| 战斗房时长 | 18.4 / 33.7 s (n=6) | 20.0~40.0 s | 偏离（目标 20.0~40.0 s） |
...
- 因子一（输出端）：A1 杂兵 TTK 中位 2.9s 超出 §14.3「初始武器 ≤2.0s」。建议校准初始武器（laohuoji）基础伤害/攻速 +20%~30% 后复测。
- 因子三（生存端）：死亡热房 F1:miniboss（1 次）。若集中于特定房型，建议校准该房型敌方弹幕密度（§7.5 上限）或弹速 -10%~15%；死亡集中于 Boss 房则复核 P2/P3 弹幕量曲线。
...
## m4-c4 附注（实现者注，非 bot 生成）

- 本批为 m4-c4 祭坛 elite_surge 分支冒烟：**任务书原定 `--trial=2026-09-03`，经 TrialSystem.pick_factors 实算该日因子为 melee_drops+narrow_vision（不含 elite_surge）**，故改用 `--trial=2026-09-05`（elite_surge+single_element）为基日期；run2 落在 2026-09-06（enemy_haste+energy_tax）= 无 elite_surge 因子的试炼对照局。
- seed 2308（elite_surge 激活）：bot 在 F1 room1 已清后经 `Altar.interact` 触发祭坛，**branch=elite**（交互改追加 1 精英、不再弹三选一；逐局表 altars 列 + stdout `BALANCE-BOT ALTAR ... branch=elite`）。遥测佐证（user://telemetry.csv）：`trial_elite_bonus,853,1,altar`——复用既有事件、第 4 列 "altar" 区分精英房波次来源（既有精英房行为 3 列行不变，如同批 `trial_elite_bonus,3027,3,`）。该局追加精英后 bot 仍清到 10 房、死于小 Boss（电磁蛛）=「更难但可玩」。
- seed 2309（无 elite_surge）：无祭坛交互行（布局无早期祭坛）= 对照局，分支门控不误触发。
- 批次结果：2 局全终局（死亡）、0 停滞、0 崩溃、0 超时（`BALANCE-BOT DONE` 行）。
- seed-base 选型说明同 m4-c4-smoke-normal.md 附注（2308 首战斗房祭坛命中种子）。
