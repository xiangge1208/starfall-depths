...
- 驱动：`tools/balance_bot.gd` 无头自动游玩（启发式走位/避弹/避爆炸域/避 hazard/环绕走位/索敌开火/概率翻滚/商店买红心/事件一律接受/三选一贪心），hero=guardian（m4-c2 --hero 参数，缺省 vanguard），headless 墙钟速率（引擎 time_scale 对本作逐拍定步长逻辑无加速效用，物理拍=真实玩家体验时长）
...
| 单局时长(均值/中位) | 2.2 / 3.1 min | 25.0~35.0 min（单局口径） | 第 1 层子集，仅参考 |

## 逐局

| seed | 结果 | 止步 | 房间 | 击杀 | 时长(min) | 金币 | 蓝晶 | 买红心 | 增益选择 | 首杀可获 | 死因 |
|---|---|---|---|---|---|---|---|---|---|---|---|
| 2211 | death | 第1层 | 0 | 3 | 1.3 | 0 | 0 | 0 | - | 否 | 穴蝠的接触冲撞（combat） |
| 2212 | stalled | 第1层 | 0 | 2 | 3.1 | 0 | 0 | 0 | - | 否 | - |
...
|---|---|---|---|
| A1 杂兵 TTK | 2.6 / 7.2 s (n=5) | ≤2.0 s | 偏离 |
...
- 因子一（输出端）：A1 杂兵 TTK 中位 2.6s 超出 §14.3「初始武器 ≤2.0s」。建议校准初始武器（laohuoji）基础伤害/攻速 +20%~30% 后复测。
