...
| 单局时长(均值/中位) | 2.3 / 3.2 min | 25.0~35.0 min（单局口径） | 第 1 层子集，仅参考 |

## 逐局

| seed | 结果 | 止步 | 房间 | 击杀 | 时长(min) | 金币 | 蓝晶 | 买红心 | 增益选择 | 首杀可获 | 死因 |
|---|---|---|---|---|---|---|---|---|---|---|---|
| 2308 | death | 第1层 | 7 | 44 | 3.2 | 27 | 5 | 0 | element_vision | 否 | 苦力虫的自爆（combat） |
| 2309 | death | 第1层 | 2 | 20 | 1.4 | 0 | 0 | 0 | - | 否 | 双刀蜥人的接触冲撞（elite） |
...
|---|---|---|---|
| A1 杂兵 TTK | 4.3 / 15.9 s (n=52) | ≤2.0 s | 偏离 |
| 精英击杀 | 11.9 / 16.6 s (n=12) | （参考） | - |
| 战斗房时长 | 23.3 / 42.0 s (n=7) | 20.0~40.0 s | 达标 |
...
- 因子一（输出端）：A1 杂兵 TTK 中位 4.3s 超出 §14.3「初始武器 ≤2.0s」。建议校准初始武器（laohuoji）基础伤害/攻速 +20%~30% 后复测。
- 因子三（生存端）：死亡热房 F1:combat（1 次）。若集中于特定房型，建议校准该房型敌方弹幕密度（§7.5 上限）或弹速 -10%~15%；死亡集中于 Boss 房则复核 P2/P3 弹幕量曲线。
...
- 本批为 m4-c4 祭坛增益分支冒烟：seed 2308 的 F1 首战斗房（room1）掷签命中祭坛，bot 经生产接口 `Altar.interact` + `Altar.choose` 交互，**branch=buff pick=element_vision**（逐局表 altars 列；三选一全部来自 buffs.json 既有池）。seed 2309 布局无早期祭坛，为一般回归对照。
- 遥测佐证（user://telemetry.csv）：`altar_spawn,1,combat_a1_01`（设施生成）、`altar_offer,1398,3`（三选一掷出）、`altar_pick,1398,element_vision`（选中落地）。
- 批次结果：2 局全终局（死亡）、0 停滞、0 崩溃、0 超时（`BALANCE-BOT DONE` 行）。
- seed-base 选型说明：2308/2309 由首战斗房祭坛命中扫描选出（前置批 seed-base=2301 全层无祭坛、2302/2303 祭坛房在 bot 死亡点之后，无法产生交互证据，故换扫种子重跑；祭坛 15% 概率本身由单测分布带钉死）。
