...
> **m4-wire 池接线冒烟口径（先读）**：本批为 `feat(m4-wire)` 卡的冒烟证据——F2 池
> 新增 C-1 移交 4 特技敌（hardshell_turtle/thorn_turret/moss_slime/seed_pitcher，
> 附录 B.2 设计楼层=A1，F1 池冻结 → 最近语义落位 F2）。命令
> `godot --headless --path . res://tools/balance_bot.tscn -- --runs=2 --seed-base=2501
> --out-md=docs/superpowers/reports/m4-wire-smoke.md --out-json=...json`。
> - 崩溃 0 / 停滞 0 / 超时 0；2/2 局止步 F1（bot 能力结构性边界，与 m4-c1-smoke
>   10/10 止步 F1 同型披露，非本卡回归）。
> - **F2 特技敌未获对局出场证据**（bot 未达 F2，ttk_rows 敌 id 全为 F1 池 4 行
>   = F1 池零改动零漂移的旁证）；池生效由 `test_scene_floor_pool_signature_guests_wired`
>   钉死 + 全量 1808/1808 绿承载，不强求对局证。
> - 池组合变更对 F2/F3 遭遇难度的影响由 B-4 的 100 局全量复跑承载评估。
> - 以下为 bot 固定模板正文（模板头注沿用的历史基线表述属模板原文，非本批状态）。
...
| 单局时长(均值/中位) | 0.9 / 0.9 min | 25.0~35.0 min（单局口径） | 第 1 层子集，仅参考 |

## 逐局

| seed | 结果 | 止步 | 房间 | 击杀 | 时长(min) | 金币 | 蓝晶 | 买红心 | 增益选择 | 首杀可获 | 死因 |
|---|---|---|---|---|---|---|---|---|---|---|---|
| 2501 | death | 第1层 | 2 | 13 | 0.9 | 0 | 0 | 0 | - | 否 | 苦力虫的自爆（combat） |
| 2502 | death | 第1层 | 1 | 10 | 0.9 | 0 | 0 | 0 | - | 否 | 苦力虫的自爆（combat） |
...
|---|---|---|---|
| A1 杂兵 TTK | 4.2 / 13.3 s (n=23) | ≤2.0 s | 偏离 |
| 战斗房时长 | 26.3 / 38.0 s (n=3) | 20.0~40.0 s | 达标 |
...
- 因子一（输出端）：A1 杂兵 TTK 中位 4.2s 超出 §14.3「初始武器 ≤2.0s」。建议校准初始武器（laohuoji）基础伤害/攻速 +20%~30% 后复测。
- 因子三（生存端）：死亡热房 F1:combat（2 次）。若集中于特定房型，建议校准该房型敌方弹幕密度（§7.5 上限）或弹速 -10%~15%；死亡集中于 Boss 房则复核 P2/P3 弹幕量曲线。
