...
- 驱动：`tools/balance_bot.gd` 无头自动游玩（启发式走位/避弹/避爆炸域/避 hazard/环绕走位/索敌开火/概率翻滚/商店买红心/事件一律接受/三选一贪心），hero=engineer（m4-c2 --hero 参数，缺省 vanguard），headless 墙钟速率（引擎 time_scale 对本作逐拍定步长逻辑无加速效用，物理拍=真实玩家体验时长）
...
| 单局时长(均值/中位) | 1.0 / 1.4 min | 25.0~35.0 min（单局口径） | 第 1 层子集，仅参考 |

## 逐局

| seed | 结果 | 止步 | 房间 | 击杀 | 时长(min) | 金币 | 蓝晶 | 买红心 | 增益选择 | 首杀可获 | 死因 |
|---|---|---|---|---|---|---|---|---|---|---|---|
| 2221 | death | 第1层 | 3 | 16 | 0.6 | 0 | 0 | 0 | - | 否 | 苦力虫的自爆（combat） |
| 2222 | death | 第1层 | 5 | 29 | 1.4 | 0 | 0 | 0 | - | 否 | 苦力虫的自爆（combat） |
...
|---|---|---|---|
| A1 杂兵 TTK | 3.5 / 9.7 s (n=45) | ≤2.0 s | 偏离 |
| 战斗房时长 | 18.1 / 30.0 s (n=6) | 20.0~40.0 s | 偏离（目标 20.0~40.0 s） |
...
- 因子一（输出端）：A1 杂兵 TTK 中位 3.5s 超出 §14.3「初始武器 ≤2.0s」。建议校准初始武器（laohuoji）基础伤害/攻速 +20%~30% 后复测。
