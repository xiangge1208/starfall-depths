...
| 项 | 值 |
|---|---|
| main tip | `a177691`（merge: m2-t36；前序 `7ac811c` m2-t29 → `469506f` docs → `a87605f` test seal） |
| 工作树 | main 检出干净（仅 `.tmp_session_extract/`、`.workbuddy/` 未跟踪）；在飞 worktree：m2-t35（T35 评审中）/ m2-t30 / m2-t37 / m2-retest |
| 全量测试 | **1335/1335 绿**，74 套件，exit 0（T36 合入后，本次亲验） |
| 测试耗时 | 5m14s |
...
| 卡 | 内容 | 分支 | 状态 | 下一步 |
|---|---|---|---|---|
| **T35** | meta 生效接线（裁定⑨；含 T32 的 17 条成就发射点 + reset_session + shop_purchase） | m2-t35 | **实现完成 `f9c5022`**（自报 1344 绿；9 键如实未接线→移交，wipe→repair 解 T15 自抵消），评审在飞 | 评审 → 修复轮（如有）→ merge |
| **T37** | 全图集合并 + A2 剪影批处理（裁定㉓，F2 draw call 超标闭环） | m2-t37 | **第五波已派**（worktree 已 ff 至 a177691；禁改 perf_probe.gd——retest 所有权） | 实现 → 评审 → merge |
...
| **T33** | 门禁预检（移交项闭环复核 + 裁定㉔ save_headless 密闭 + M1 补录 4 项核销 + T36 评审 Minor-5 A2 Boss hp 断言补钉） | — | 从未派发 | 全部卡合并后 |
