...
> **2026-08-31 下午（同会话续 2）**：T35 评审 **Approved**（零 C/I；10 个未接线键全部经 grep 证实无消费者、合法移交）→ 已合 main，全量 **1358/1358 绿**（75 套件，亲验）；T33 预检提前派出（见裁定㉗范围扩张）。在飞：T37 / T30 / 复测 / T33。
> **2026-09-01 第五次接管（配额窗口 1308 再杀三 Agent）**：①T37 修复轮 `011f671` 范围复评通过（Important-1 同机探针两路定案：参与集方案 147.6/148.5 余量 ~1% 否决 → 折叠方案落地余量 32.5%；编排者独立复算像素证据 101,583≈声明 103,780、径向剖面光圈环带增亮成立；Minor 3/4/5/7/8 代码核验 + 2/6 勘误在卷）→ 分支亲验 **1357/1357 绿** → 合 main **1380/1380 绿**（76 套件）。②T33 五提交编排者核证据通过（六发射点 file:line 抽查全中；13 例接线测试 TestSaveSeal 密封口径扎实；demolition blocked / collector 数据源缺口 / full_roster 休眠三项如实入账不造机制）→ merge-tree 零冲突预检 → 合 main 终 gate **1399/1399 绿（77 套件，0 orphans，exit 0）**（T33 报告汇总行「1368」系誊写笔误，净增 19 例以合并态亲验为准）。③复测卡接管直跑（裁定㉚）：探针现代化合入 + main 窗口探针 **PERF VERDICT PASS**（F1 101.4 / F2 102.2 / F3 102.1 avg ≤150；证据 `t37-evidence/m2_perf_main_2026-09-01.json`）。报告归档：`task-33-report.md` / `task-37-report.md` / `t37-evidence/`（4 PNG + 4 JSON）。
...
| T37 | 全图集合并 + A2 光圈批处理 + 修复轮弹幕可读性折叠（裁定㉓㉙） | （m2-t37 merge，含 `011f671`） | 评审 Approved 1I+7m 全闭环：F2 157.2→**105.6**（首跑）→折叠终态 **102.2 avg**（main 复测）；参与集方案探针否决（148.5 贴线）；预警纹 LIT_ITEM_MASK + 弹幕 self_modulate 折叠；诊断脚本入库 `tools/diag_light_attribution.*`；证据归档 `t37-evidence/` |
| T33 | 门禁预检（裁定㉗㉔：成就 16 链补线 + save_headless 六套件密封 + M1 补录③④ + T36/T35 Minor 批修 + 移交复核） | （m2-t33 merge，5 提交） | 编排者核证据通过；22 条 M2 成就 = 5 既活 + 16 新接线 + 1 blocked（demolition 机制不存在，如实入账）；TestSaveSeal 密闭器 + 极限脏档破坏性验证抓 2 处均修；M1 补录③ 死亡确认锁落地、④ 无缺口核销；8 派味特技 0/8 + 英雄被动 5 条 data-only → M3 清单；+19 测试 |

### 待处理（无——M2 全卡收口）

| 卡 | 内容 | 分支 | 状态 | 下一步 |
|---|---|---|---|---|
| ~~复测~~ | T28 胜率带复跑 + TTK 正式表 + T29 密房复测 | m2-retest | **完成归档**（2026-09-01）：T29 密房复测 = main 窗口探针 PASS（F1 101.4/F2 102.2/F3 102.1 avg）；T28 胜率带批 3001..3010 = 0 崩溃、0 胜/7 死/3 超时全止步 F1（bot 能力瓶颈：自爆虫 6/7 + 门槛震荡 3——telemetry 实证非游戏死锁）→ 胜率带 bot 口径不可评、真人权威；TTK 表 = A1 杂兵 3.7s 中位 vs ≤2.0s **偏离 1.85×**（M3 校准点）、战斗房 23.9s 带内达标。报告 `m2-balance-rerun-2026-09-01.md`（含编排者增补修正工具模板过时叙事 + M3 三行动项） | 已入 m2-gate |
| ~~T34~~ | M2 门禁 | — | **已执行**（2026-09-01，编排者）：双报告 + 综合裁定 **GREEN（附条件）** + tag `m2`；集成守卫八项（1399×2 确定性、烟测、9000 装配校验、探针 PASS、存档 v2、导出冒烟、平衡复跑、卫生）见 `m2-gate-integration.md`；真人十项 checklist **已搁置**（2026-09-01 裁定㉝）见 `m2-gate-playtest.md`（原裁定㉛） | ~~真人试玩 → M3 前置~~ → **合并 m3-prelude → M3 计划**（真人试玩搁置，用户自测回报） |
...
30. **复测卡执行口径 + 两卡合并记录**（2026-09-01，编排者接管直跑）：①**T29 密房复测判定完成**——T37 修复轮探针（`t37-evidence/m2_perf_fixround_t37.json`）+ main 合并态窗口复测（`t37-evidence/m2_perf_main_2026-09-01.json`：F1 combat_a1_03 101.4 / F2 combat_a2_01 102.2 / F3 combat_a3_08 102.1 avg，max 187/166/176；PERF VERDICT PASS）双层证据，T26 后 densest 按层池自动选型生效；m2-retest 未提交探针改动（注释勘误 + A3 注入 workaround 移除）经核为合法现代化，ff 至 main 后以 `080d0cb` 合入。②**T28 胜率带复跑口径**：最终 main（T33 合并后 1399 绿态）上独立新种子 3001..3010 跑 10 局（与 8-31 校准批 2001..2010 区分样本、避免首杀标记/解锁池跨批混淆；批前清 save_headless 与首批同起点）；TTK 正式表从该批出（三层全内容首次可评）。③**合并数字亲验**：T37 分支 1357/1357（75 套件）→ main 合后 1380/1380（76 套件）→ T33 合后 **1399/1399（77 套件，0 orphans，exit 0）**；T37 报告「预期 1355」与实际 1357 差异系报告估算笔误（MageGuardian 2 例恢复未计入），以提交与亲验为准。④T33 报告「1368/76」汇总行系誊写笔误（净增 19 例以合并态 1399 亲验为准），不影响证据链。
31. **M2 门禁裁定（2026-09-01，T34，GREEN 附条件）**：①集成守卫八项——1399/1399 ×2 确定性一致、启动烟测 0 错、地牢校验 1000 契约回归 + 3000 种子×3 生态 9000/9000（`validate_dungeon.gd` 参数化 `--seeds/--floors`，默认契约逐字节不变亲验）、窗口探针 PERF VERDICT PASS（F1 101.4/F2 102.2/F3 102.1 avg ≤150）、存档 v2 往返含于全绿、T30 导出冒烟引用、平衡复跑完成、卫生披露；②**两项量化偏离如实入账不判 FAIL**：A1 杂兵 TTK 3.7s vs ≤2.0s（bot 口径，vanguard+laohuoji 最弱组合，M3 校准点先采真人数据）+ 胜率带 bot 不可评（bot 止步 F1：自爆虫 6/7 死因 + 门槛震荡 3 超时——telemetry 实证 `floor_enter` 逐帧交替系 bot 启发式门界局部均衡，非游戏死锁；GDD §14.3 带本为玩家成长曲线，真人权威）；③裁定 **GREEN（附条件）**沿 M1 先例：tag `m2` 不因真人项阻塞，真人十项 checklist（完整 3 层通关/三层 Boss 三阶段/藤蔓巨像 90~150s〔M1 补录①〕/A2 光圈弹幕可读性〔裁定㉙〕/全房型/三选一/熔铸/图鉴成就/死亡锁/主观 ≥3/5）为 **M3 开工前常设验证项**，FAIL 则按严重度定 M3 首卡或回滚 m3-prelude 合并；④M3 移交清单固化入 `m2-gate.md` §5（平衡三行动项 + T33/T35 移交 + 双平台 60fps + 模板缓存）。

32. **M1/M2 全程复审审计收口（2026-09-01，用户指令「再次完整检查」）**：三路并行子 Agent 复核（数据一致性 11 项全对齐 / 移交链 17 项逐条代码级核实 / 1399 绿+卫生亲跑）后，内容遗漏 8 项按严重度处置——**随 `fix(m2-audit)` 系列提交代码修复 6 项**：①A2/A3 波次名录分层（`FLOOR_TRASH` 附录 B.2 特有种 12 行/层；T26 披露「T36 评估」断链收口；池行可构造性测试防软锁）②精英词缀 §12.3 楼层递进（`elite_affixes_for_floor`：A1 无/A2 一/A3 二，层派生 `SALT_ELITE`；行内固定 swift+berserk 降为缺省）③掉落权重 §8.2 分源修复（宝箱 30/35/22/10/3、精英奖励 10/30/35/20/5 入 `ShopLogic.SOURCE_RARITY_WEIGHTS`；删除漂移键名 `LOOT_RARITY_WEIGHTS`——绿档此前永不被直抽 ~3.2% vs 设计 30%）④`--script` 回退过滤 locked（T6 评审移交过期项；回退池 115→66 与生产同口径）⑤EnemyLaser 弹幕预算重估结论入档（束不入 400 上限——炮台「冷却≥束寿命」行不变量 + Boss 单拍 1 束；测试钉死；T7 评审 m4 悬置承诺收口）⑥熔铸台位置模板行驱动（combat 行可选 `forge:[x,y]` 像素偏移字段，24 行录 [0,56] 行为保持；T25 披露收口）。**文档补录 2 项**：⑦I 束「暴击/元素弹专用帧」「美术 QA 三重校验」无声收窄 → roadmap/M3 清单显式补录；⑧附录 I.4「唯二放宽」扩为三项（翻滚 CD 0.12，T2 评审 advisory 后半）。**另两处门禁级缺口（经济模拟零执行、单层/单局时长零数据）非本收口范围**，已补录 M3 清单与 roadmap 门禁操作化脚注，M3 计划须立卡。记录更正：m2-gate §5 旧「5 条 data-only 被动」中 hawk_eye 实自 M1-T5 已实现（真 4 条），M3 清单口径照改。

33. **真人试玩搁置（2026-09-01，用户指令）**：M2 门禁 GREEN 附条件中的真人十项 checklist（及 M1 期同源附条件）转 **SHELVED**——用户将自行游玩并按实际体验回报问题，流程不再反复提及、不作为任何前置。流程影响：①m3-prelude 合并与 M3 计划启动不再以真人试玩为前置，下一步 = 直接合并 m3-prelude → M3 计划；②「先采真人数据」的 M3 校准点（TTK / 胜率带 / 自爆虫死因 / 单层单局时长两带）改为**用户自测反馈驱动**——用户回报前按 bot/遥测口径先行；③roadmap 硬规则 5「每 5 卡试玩员抽查」同批挂起（M1/M2 全程复审曾判其静默失效，现正式搁置）；④已标记文档：`m2-gate.md` / `m2-gate-playtest.md`（转自测参考清单）/ roadmap 门禁操作化④与硬规则5；历史门禁报告（m0/m1 各报告、m2-gate-integration）作为时点记录不改写。恢复 checklist 验收门需用户明示。

34. **m2 tag 重打（2026-09-01，用户指令）**：审计收口（裁定㉜）+ 真人项搁置（裁定㉝）落地后，原 tag `m2`（@414843a，关门时点记录）删除重打至收口态 HEAD。重打前关门级复验（全部在 tag 目标提交亲跑）：全量 **1407/1407 绿**（77 套件，0 orphans，exit 0）+ 启动烟测 0 SCRIPT ERROR + 地牢校验 **1000/1000**（M1 契约）与 **9000/9000**（门禁口径 3000 种子×3 生态）。`m0`/`m1` 保留为历史里程碑边界标记不动（m1 门禁态已被 M2 收官消费；重打仅针对被审计修复超越的 m2）。原 414843a 的门禁八项记录仍以 `m2-gate-integration.md` 文档为准。
