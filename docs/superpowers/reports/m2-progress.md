...
| 卡 | 内容 | 分支 | 状态 | 下一步 |
|---|---|---|---|---|
| **T25** | 熔铸台场景接线 | `m2-t25` | 实现完成，**独立评审中**（`b331bce`+`c0ba77a` 已 merge main；+1012 行 / 35 测试 / 自述 1011 绿） | 评审 → 修复 → merge（解锁 T28） |
| **prophet** | 星陨先知 + 隐藏门 | `m2-prophet` | 0 提交，worktree 有**未提交半成品**（boss+测试+6 文件接线） | 恢复代理已派（salvage → merge main → 补完） |
| **T26** | 挑战房灾厄 + **A2/A3 模板 ×16（裁定⑭）** + 房型矩阵复核 + 小Boss缩放/晶柱拆分/冰面zone视觉（移交表） | `m2-t26` | 0 提交，worktree 有**未提交半成品**（calamity UI+测试） | 第二波恢复实现 |
| **T28** | Balance Bot 全层回归 | `m2-t28` | **0 提交**（配额中断） | 重派（**依赖 T25 合并**） |
| **T29** | §18.3 全指标压测 | `m2-t29` | **0 提交**（配额中断） | 重派（T27 已合，可派） |
| **T31** | 蓝晶结算 + 存档 migration v2 | `m2-t31` | 0 提交，worktree 有**未提交半成品**（save v2 + 结算接线 ~224 行） | 恢复代理已派 |
| **T32** | 成就系统 22 激活 | `m2-t32` | 0 提交，worktree 有**未提交半成品**（achievement_system + toast + save 接线） | 恢复代理已派 |
