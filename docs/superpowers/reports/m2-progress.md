...
| 项 | 值 |
|---|---|
| main tip | `89b5cfa`（merge: m2-t19 magma tyrant into main） |
| 工作树 | 干净，仅 `.tmp_session_extract/`（本次排查临时目录，可删） |
| 全量测试 | **1097/1097 绿**，66 套件，0 errors / 0 failures / 0 flaky / 0 orphans，exit 0 |
| 测试耗时 | 2m13s |
...
| 卡 | 内容 | 分支 | 状态 | 下一步 |
|---|---|---|---|---|
| **T25** | 熔铸台场景接线 | `m2-t25` | 评审 NEEDS FIXES（`5b1061b`，Imp×2/Min×4）→ 修复轮 `0e25990` 已落（**前会话存留代理自动执行**，追认裁定⑮⑯；自述 1142 绿）；**Important-1 craft_x 接线仍未做** | 复核 → merge（解锁 T28） |
| **prophet** | 星陨先知 + 隐藏门 | `m2-prophet` | **实现完成待评审** `c4ebc1a`（1116/1116 = 1097+19；半成品全保留；披露：共鸣口径宽松化/共鸣斩鸭子接缝/drops 设计补充） | 独立评审 → merge |
| **T26** | 挑战房灾厄 + **A2/A3 模板 ×16（裁定⑭）** + 房型矩阵复核 + 小Boss缩放/晶柱拆分/冰面zone视觉/召唤物残留收口（T25 评审 Imp-2 移入） | `m2-t26` | **恢复实现中（第二波已派）**，worktree 半成品（calamity UI+测试） | 分里程碑提交 |
| **T28** | Balance Bot 全层回归 | `m2-t28` | **0 提交**（配额中断） | 重派（**依赖 T25 合并**） |
| **T29** | §18.3 全指标压测 | `m2-t29` | **0 提交**（配额中断） | 重派（T27 已合，可派） |
| **T31** | 蓝晶结算 + 存档 migration v2 | `m2-t31` | **实现完成待评审** `65a4c27`（1117/1117 = 1097+20；披露：回退半成品「进门入账」保 GDD 死亡减半口径；Boss 首杀 +300 有 GDD §14 出处；修复半成品 floor_scene 编译错误） | 独立评审 → merge |
| **T32** | 成就系统 22 激活 | `m2-t32` | **实现完成待评审** `e18f3be`（1131/1131 = 1097+34；披露：SAVE_VERSION=2 与 T31 同值并集可解；5 条经 EventBus 实时激活、17 条待 T35 发射点；修复 int 强转恒真 bug） | 独立评审 → merge |
| **T30** | Windows 导出冒烟 + Android 链预通 | — | 从未派发 | 新建 worktree 派发 |
| **T33** | 门禁预检（移交项闭环复核） | — | 从未派发 | 全部卡合并后 |
| **T34** | M2 门禁（双报告 + 裁定 + tag） | — | 从未派发 | T33 之后 |
| **T35** | meta 生效接线（裁定⑨） | — | 已立项待派 | 与 T18 错开 `run_root.gd`，可派 |
...
18. **前会话存留代理（僵尸）事件记录**：2026-08-31 12:55–13:07+，一存活代理在 `.worktrees/m2-t25` 自主执行修复轮并读走本编排者评审报告（`5b1061b`）后提交 `0e25990`。处理：不派对抗代理，监控其产出；其裁定编号以台账为准重编。若其在 main 上自行 merge 即回滚并接管。
