...
| 卡 | 内容 | 分支 | 状态 | 下一步 |
|---|---|---|---|---|
| **复测** | T28 胜率带复跑 + TTK 正式表 + T29 A2/A3 密房复测（T26 已合前提） | m2-retest | 探针现代化已合（`080d0cb`）；**T29 密房复测已完成**（main 窗口探针 PERF VERDICT PASS，见 2026-09-01 会话注）；T28 胜率带 10 局批（种子 3001..3010）执行中 | 批完成 → TTK 正式表 → 报告增补 → 归档 |
| **T34** | M2 门禁（双报告 + 裁定 + tag；含 T29 渲染侧真机复测 + M1 补录第 1/2 项 + 裁定㉔② 真人 Boss 房段试玩——T36 已合，具备条件） | — | 集成守卫材料齐（1399 绿终 gate + 确定性/烟测/千种子三件套 + 探针 PASS + T30 导出冒烟） | 复测批归档后出双报告 + tag `m2`；真人项列用户待办 |
