...
| `boss_defeated(boss_id: String, floor_idx: int)` | 新声明 | M2-T33（BossBase/RoomCombat 死亡点发射） |
...
|---|---|---|---|---|---|---|---|---|
| 1 | `first_lamp` | 初次点灯 | 首次击败 A1 Boss | 100 | M2 | `boss_defeated` | 信号参数 `floor_idx` | `floor_idx == 1` |
...
| 15 | `nitpicker` | 鸡蛋里挑骨头 | 用投掷武器击败 A1 Boss | 100 | M2 | `boss_defeated` | `kill` 行武器 id → `GameDB.weapons_all[..].category` | `floor_idx == 1 && category == "throw"` |
...
| 20 | `nightmare_dawn` | 噩梦黎明 | A3 无死亡击败 Boss | 300 | M2 | `boss_defeated` | 信号 `floor_idx` + Telemetry `death` 会话计数 | `floor_idx == 3 && deaths == 0` |
...
> 蓝晶合计：M2 22 条 = 3350；M3 2 条 = 300（试炼 ×1.5 后口径归 M3）。
...
| 存档字段（v2 新增） | 写入方 | 消费方 |
|---|---|---|
| `counters.kills_total / crafts_total / resonances_total / gems_earned_total / purchases_total / floor_clears[1..3] / challenge_rooms_total / trials_total` | T25（会话结束入档聚合） | T20 图鉴（J.2 六类）+ T33 成就（熔铸匠/挑战者/试炼×2） |
