...
数值基调对照 GDD §14：局内增益单条为 +6~12%（暴伤 +50%、异常 +25%），永久天赋单节点 ≤ +10%（暴伤 0.25 / 磁吸 0.30 / 翻滚冷却 0.12 三项按自身刻度放宽并注明——第三例外系 m2-audit 补记〔T2 评审 advisory 后半收口〕：蓝系 `blue_adrenaline` 的 −12% 走 `roll_cd_pct` 减法乘区，该键机器上限 0.15、10% 基调内无可感档位）；满系红约等于 3~4 条稀有增益的常驻化，与「点满一系 ≈ 14h 中值投入」的代价匹配。
