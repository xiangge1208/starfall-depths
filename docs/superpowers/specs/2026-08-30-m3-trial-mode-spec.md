...
主菜单 → 试炼面板（日期 + 因子预览 + 今日最佳 + 历史 + 开始）→ 选角 → 局内：HUD 左上层数旁增 2 个因子小图标（图标素材 = `art/generated/trials/factor_<id>.png`，P0-6 已先行；悬浮显示文案，触屏长按显示）→ 死亡/胜利结算面板标题带「每日试炼」徽标（`trial_medal.png`）+ 倍率明细行（基础 X × 1.5 = Y）→ 返回试炼面板。
