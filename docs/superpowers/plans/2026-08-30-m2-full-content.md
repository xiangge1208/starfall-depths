...
## 附 2：M1 全程复查补录（2026-08-31，T34 门禁前必须闭环）

> 来源：M1 计划/实现/门禁全程复查。以下 4 项在 M1→M2 交接中掉出跟踪网，归入 T34 门禁预检清单逐项核销；其中第 1、2 项建议提前执行，不要等 T34。

1. **藤蔓巨像 90~150s 时长验收复活**：M1-T13/T26 的验收标准，m1-gate 裁定表静默移除且本计划 T34 未含。T34 试玩员 checklist 补"藤蔓巨像战斗时长 90~150s（可击败、三阶段演出）"。
2. **真人试玩提前**：M1 终审建议与 m1-gate 附条件均为"M2 开工前第一项"，实际落在 T34（M2 末尾）——终局链路的真人手感问题将在 33 张卡叠完后才暴露。建议最近一个窗口先做真人 Boss 房段（不必完整一局），结果反哺后续 Boss 卡。
3. **死亡结算确认输入锁**：M1-R2 注记①——结算面板对离散单发按键偶发无响应（enter/space/esc/点击均不灵，连发稳定）。「F-2 死亡回顾完整」卡（正文 Task 23）实现时给确认加 ~0.5s 输入锁+轮询。
4. **跨局遥测留存**：DeathRecorder.reset() 清 Telemetry 会话文件导致旧局不可回溯（M1-R2 注记②）。「F-2 死亡回顾完整」卡一并裁定口径（会话文件轮转或开局另存），不再静默丢弃。

> 另注：本计划波次总表与正文卡号在后半段存在错位（如「门禁预检」仅存在于波次表 T33，正文无此卡；「F-2 死亡回顾」波次表 T24 vs 正文 Task 23；「蓝晶结算」正文 Task 25 与 Task 32 重叠）。引用卡时请同时核对正文标题。
