...
> 审计补记 2026-09-01（M1/M2 全程复审）：I 束「暴击/元素弹专用帧」「美术 QA 三重校验」两项未随 M2 计划落卡（无声收窄），已显式移交 M3（见 §6 M3 行）；其余项已随 M2 T17/T21/T27 交付。另：门禁操作化第 3 条的「蓝晶节奏三点抽样（经济模拟）」与「单层/单局时长两带」在 M2 门禁中无数据（bot 止步 F1、模拟未建）——随 M3 计划补立经济模拟卡，真人 checklist 补计时项。
...
4. 试玩员：真人完整通关一局（3 层含 Boss）+ 死亡可归因复查 + S0 复测项闭环；
