class_name AutoAim
extends RefCounted
## 自动瞄准纯逻辑（GDD §5.1 触屏辅助，m4p 修订：全向 360° 就近索敌，桌面/触屏同口径）。
##
## 生产接线：PlayerDriver 在无显式右杆输入且 auto_aim=true 时收集存活敌人，
## 调本类以 cone_deg=360 选出全向最近目标——触屏锁定即自动开火，桌面锁定朝向、
## 开火仍由 fire 键控制；显式触屏右杆/手柄右杆优先。
## 返回零向量表示「无目标且无既有瞄准」，调用方不开火。
##
## 选目标规则：先滤掉锥外目标（生产传 360° = 不过滤），再在锥内取最近者；
## 距离完全相同才按输入顺序稳定取前者。无目标 → -1。cone_deg 默认 60 保持不动
## （bot lead 路径与 test_auto_aim 零漂移钉测以默认参调用）。
